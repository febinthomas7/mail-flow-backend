  ...
  try {
    ...
    if (generationOptions && generationOptions.html) {
      const enrichedData = createTags({
        name: generationOptions.receiverNames
          ? generationOptions.receiverNames[
              index % generationOptions.receiverNames.length
            ]
          : "Customer",
        email: targetEmail,
        invoice: generationOptions.invoices
          ? generationOptions.invoices[
              index % generationOptions.invoices.length
            ]
          : null,
    ...
    const command = new SendRawEmailCommand({
      RawMessage: { Data: compiledMessage },
    });
...
const sendEmail = async (req, res) => {
  ...
  res.status(202).json({
    success: true,
    message:
      "Batch processing initiated. Emails are sending in the background.",
  ...
  (async () => {
    try {
      console.log(
        `[BACKGROUND JOB] Starting email blast for ${targets.length} targets...`,
      );
      ...
      console.log(
        `[BACKGROUND JOB] Finished processing ${targets.length} emails.`,
      );
      ...
      console.error(
        "[BACKGROUND JOB ERROR] Email queue failed:",
